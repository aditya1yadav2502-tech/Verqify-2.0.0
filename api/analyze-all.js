...

{
  ...
  "code_quality": {
    ...
    "signals": { "has_error_handling": true, "has_meaningful_variable_names": true, "has_modular_structure": true, "has_comments_or_docs": true, "has_consistent_style": true, "has_tests": false },
...
function calculateBreadth(analyses) {
  // Fixed: collect unique skill names across all repos instead of using Object.keys on an array
...
async function analyzeAllRepos(githubUsername, accessToken) {
  const headers = {
    Authorization: `Bearer ${accessToken}`,
    Accept: 'application/vnd.github.v3+json',
    'User-Agent': 'Verqify-Analysis-Engine/1.0',
  ...
  // Filter forks, tiny repos, and GitHub Pages repos
  ...
  console.log(`[analyzeAllRepos] Analysing ${meaningfulRepos.length} repos for @${githubUsername}`);

  // Analyse each repo in parallel
  ...
  if (valid.length === 0) throw new Error('All repository analyses failed');

  // Aggregate skills across repos with hierarchy upgrade
  const skillMap = {};
  ...
  for (const analysis of valid) {
    for (const skill of analysis.skills_detected || []) {
      ...
      if (!skillMap[key]) {
        ...
        // Upgrade to stronger evidence level if found
  ...
  // Calculate 5-dimension fingerprint
  const dimensions = {
